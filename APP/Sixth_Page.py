...
def app():
    ...
    forty = df[['YEAR','SPORT','NAME','GENDER','AGE','40 M SPRINT']].dropna().drop(df.index[df['40 M SPRINT']==0], inplace=True)
    df.drop(df.index[df['40 M SPRINT']==0], inplace=True)
    ...
    if chose == "1":
       ...
       if st.checkbox("Done"):
          ...
          file = a[['YEAR','SPORT','NAME','AGE','BMI','BODY FAT','MAXIMUM PUSH UP','1 MIN CURL UP','CMJ','SBJ','TOTAL','SIT & REACH','ILLINOIS/L','ILLINOIS/R','10 M SPRINT','20 M SPRINT','40 M SPRINT','YOYO TEST','NOTE']] 
       ...
       if st.checkbox("Done"):
          ...
          file = a[['YEAR','SPORT','NAME','AGE','HEIGHT','WEIGHT','BMI','BODY FAT','MAXIMUM PUSH UP','1 MIN CURL UP','CMJ','SBJ','TOTAL','SIT & REACH','ILLINOIS/L','ILLINOIS/R','10 M SPRINT','20 M SPRINT','40 M SPRINT','YOYO TEST','NOTE']] 
          st.write(file.set_index('NAME'))
          st.download_button(label='Download Fitness Test Result',data=file.to_csv(),mime='text/csv', file_name='Fitness Test.csv')
    elif chose == "3":
